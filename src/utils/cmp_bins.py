...
from src.utils.config import ps_threshold, ida_pro_path, bindiff_path
...
def ida_bin_export_info(bin_path: str, bin_export_path: str):
    ...
    cmd = 'sudo {} -A -OBinExportModule:{} -OBinExportAlsoLogToStdErr:FALSE -OBinExportAutoAction:BinExportBinary {}'.format(
        ida_pro,
        bin_export_path,
        bin_path
    )
...
def get_bindiff_similarity(bin1, bin2):
    ...
    print("s:%f, c:%f" % read_bindiff_export(output_file_basename))
